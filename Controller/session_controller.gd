...
func _init_navmesh() -> void:
	if !navigation_region or !navigation_region.navigation_polygon:
		return
	...
	var bounds = _get_navmesh_bounds(nav_poly)
...
func _get_navmesh_bounds(nav_poly : NavigationPolygon) -> Rect2:
	# Calculate bounds from navmesh
	...
	if min_pos.x == INF:
		push_warning("NavigationPolygon has no vertices - cannot calculate bounds")
		pass
...
# --- Accessors ---
func get_terrain() -> HexGrid: return hex_grid
func get_session_duration() -> float:
	return (Time.get_ticks_msec() / 1000.0) - session_start_time if session_active else 0.0
...
func _print_stats() -> void:
	...
	print("Grid Stats:\n  Dimensions: %dx%d\n  Total Cells: %d\n  Enabled: %d\n  Disabled: %d\n  Hex Size: %.1fpx" % [
		stats.grid_dimensions.x, stats.grid_dimensions.y, stats.total_cells,
		stats.enabled_cells, stats.disabled_cells, stats.hex_size
	])

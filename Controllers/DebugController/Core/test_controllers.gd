...
func _ready():
	...
	var hex_grid_ctrl = load("res://Controllers/HexGridController/Core/hex_grid_controller.gd").new()
	if hex_grid_ctrl:
		print("  ✓ HexGridController instantiated")
	else:
		print("  ✗ HexGridController FAILED")

	var nav_ctrl = load("res://Controllers/NavigationController/Core/navigation_controller.gd").new()
	if nav_ctrl:
		print("  ✓ NavigationController instantiated")
	else:
		print("  ✗ NavigationController FAILED")

	var debug_ctrl = load("res://Controllers/DebugController/Core/debug_controller.gd").new()
	if debug_ctrl:
	...
	var session_ctrl = load("res://Controllers/SessionController/Core/session_controller.gd").new()
	if session_ctrl:
